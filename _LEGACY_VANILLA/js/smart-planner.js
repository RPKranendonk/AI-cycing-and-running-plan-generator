...
async function calculateSmartBlock() {
    ...
    try {
        ...
        if (isCycling) {
            ...
            // Use highest volume week as starting point
            const startVol = maxVol.toFixed(1);

            // Calculate long run percentage based on volume tier:
            // < 40km: 45% | 40-90km: 35% | > 90km: 25%
            let lrPercentage = 0.35; // Default for 40-90km
            if (maxVol < 40) lrPercentage = 0.45;
            else if (maxVol > 90) lrPercentage = 0.25;

            const startLRRaw = maxVol * lrPercentage;
            const startLR = (Math.ceil(startLRRaw * 2) / 2).toFixed(1); // Round to nearest 0.5

            if (document.getElementById('target-volume')) document.getElementById('target-volume').value = startVol;
            if (document.getElementById('target-long-run-run')) document.getElementById('target-long-run-run').value = startLR;
        }
        ...
        // Update the simplified recommendation display
        const reason = needRest
            ? `Avg weekly increase ${avgIncrease.toFixed(1)}% > 5%`
            : `Avg weekly increase ${avgIncrease.toFixed(1)}% is moderate`;
        if (window.updateRestWeekRecommendation) {
            window.updateRestWeekRecommendation(needRest, reason);
        }
