...
`;
    ...
    return `
ROLE: You are an elite endurance coach creating a specific workout plan for Week ${params.weekNumber} of a ${params.totalWeeks}-week block. You prescribe holistic training including running, strength, core, and cross-training.
...
VOLUME TARGETS:
- Total Volume: ~${params.totalVolume} km
- Long Run: ${params.longRunDistance} km on ${params.longRunDay} (MANDATORY)
- Remaining Volume: ${(params.totalVolume - params.longRunDistance).toFixed(1)} km to be distributed across other days.
...
4. **MINIMUM DISTANCE**: All RUNNING sessions must be at least 5km. If a run would be shorter, increase it to 5km or change it to cross-training/rest.
5. **NON-RUNNING DAYS**: On days without running, you MUST propose one of: Rest, Strength, Bike, Pilates, or Cross Training. Do not leave days blank or just "Rest" unless appropriate for recovery.
6. Balance remaining volume across other available days.
7. If the athlete has fewer available days than recommended, prioritize key sessions (Long Run, Tempo/Intervals) and combine easy volume if safe.
...
async function deleteBlockWorkouts(weekIndices) {
    ...
    for (const idx of weekIndices) {
        await deleteWorkoutsForWeek(idx);
