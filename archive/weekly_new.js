// ==========================================
// PART 1: PLAN GENERATION LOGIC (NEW)
// ==========================================

/**
 * PHASE 1: MACRO-LOGIC (Duration Calculation)
 * Calculates the length of each phase based on total weeks available.
 */
function calculatePhaseDurations(totalWeeks) {
    // 1. Determine Taper (Sharpening)
    // Exception: If W < 9, Taper = 2 weeks. Otherwise 3.
    let taperWeeks = totalWeeks < 9 ? 2 : 3;

    // 2. Calculate Remaining Weeks for training
    let remaining = totalWeeks - taperWeeks;

    // Safeguard for extremely short plans
    if (remaining < 1) {
        return { phase1: 0, phase2: 0, phase3: remaining, taper: taperWeeks };
    }

    // 3. Calculate Phase 3 (Race Reality)
    // Formula: Round(R * 0.4), Minimum 3
    let phase3 = Math.round(remaining * 0.4);
    if (phase3 < 3) phase3 = 3;

    // 4. Calculate Phase 2 (Engine Development)
    // Formula: Round(R * 0.35), Minimum 2
    let phase2 = Math.round(remaining * 0.35);
    if (phase2 < 2) phase2 = 2;

    // 5. Calculate Phase 1 (Structural Integrity)
    // Formula: R - (Phase 3 + Phase 2)
    let phase1 = remaining - (phase3 + phase2);

    // --- THE SHORT PLAN OVERRIDE ---
    // If Phase 1 <= 1 week, assume "Base is done"
    if (phase1 <= 1) {
        console.log("Short plan detected: Activating Base Override");
        phase1 = 0;

        // Redistribute Remaining: 60% Race Reality, 40% Engine
        phase3 = Math.round(remaining * 0.6);
        phase2 = remaining - phase3; // Ensure the sum equals exactly 'remaining'

        // Enforce minimums again if possible
        if (phase3 < 1) phase3 = 1;
    }

    // Final sanity check to ensure we didn't exceed total weeks
    let currentTotal = phase1 + phase2 + phase3;
    if (currentTotal > remaining) {
        let diff = currentTotal - remaining;
        if (phase2 > diff) {
            phase2 -= diff;
        } else {
            let remainder = diff - phase2;
            phase2 = 0;
            phase3 -= remainder;
        }
    }
    ...
    return {
        phase1: phase1, // Structural Integrity
        phase2: phase2, // Engine Development
        phase3: phase3, // Race Reality
        taper: taperWeeks // Sharpening
    };
}

/**
 * PHASE 2: MICRO-LOGIC (Week Naming)
 * Determines the focus name based on position in block.
 */
function getWeekFocusName(position, blockLength, phaseType) {
    // Special Handling for Taper Phase
    if (phaseType === "Sharpening") {
        if (position === blockLength) return "Race Week";
        return `Taper Phase ${position}`;
    }

    // The Technical Approach Logic
    if (blockLength === 2) {
        // Compressed Block
        return position === 1 ? "Accumulation" : "Overreach";
    }
        ...
        // Very Long Block - Repeat Accumulation/Progression
        if (position === 1) return "Acclimation";
        if (position === blockLength) return "Adaptation";
        if (position === blockLength - 1) return "Overreach";
        return "Progression";
    }
 ...
 * Combines Macro and Micro logic to build the plan object.
 */
function generateTrainingPlan(totalWeeks, goalTime, raceDateStr) {
    const phases = calculatePhaseDurations(totalWeeks);
    console.log("Plan Structure:", phases);

    const plan = [];
    let currentWeek = 1;

    // Calculate Start Date (Backwards from Race Date)
    const raceDate = new Date(raceDateStr);
    // Determine the start date of the entire plan
    const planStartDate = new Date(raceDate);
    planStartDate.setDate(raceDate.getDate() - (totalWeeks * 7));

    // Helper to generate a block of weeks
    const addBlock = (length, phaseName, phaseType) => {
        if (length <= 0) return;

        for (let i = 1; i <= length; i++) {
            const focus = getWeekFocusName(i, length, phaseType);
            let isRaceWeek = (phaseType === "Sharpening" && i === length);

            // Calculate start date for this specific week
            let weekStart = new Date(planStartDate);
            weekStart.setDate(planStartDate.getDate() + ((currentWeek - 1) * 7));

            plan.push({
                week: currentWeek,
                phase: phaseName,
                blockType: phaseType === "Sharpening" ? "Taper" : (isRaceWeek ? "Race" : "Build"),
                positionInBlock: i,
                blockLength: length,
                focus: focus,
                isRaceWeek: isRaceWeek,
                startDate: weekStart.toISOString(), // Needed for Intervals.icu
                // IMPORTANT: Placeholders for volume. 
                // You should call your 3:1 volume logic function to populate these
                rawKm: 30 + (currentWeek * 2),
                longRun: 10 + currentWeek
            });
            currentWeek++;
        }
    };
    ...
    addBlock(phases.phase1, "Structural Integrity", "Base");
    addBlock(phases.phase2, "Engine Development", "Build");
    addBlock(phases.phase3, "Race Reality", "Peak");
    addBlock(phases.taper, "Sharpening", "Sharpening");
...
// ==========================================
// PART 2: UI & INTERACTION LOGIC (EXISTING)
// ==========================================
...
function getSmartPhaseName(position, length, type) {
    // Note: This logic is partially duplicated in getWeekFocusName above
    // but kept here for UI compatibility if called separately.
    ...
    if (length === 2) {
        // Compressed Block
        const names = ['Accumulation', 'Adaptation'];
        return names[position - 1] || 'Build';
    }
    else if (length === 3) {
        // Short Block
...
async function callOpenAI(prompt, weekIndex, availability) {
    showToast("🤖 Asking OpenAI to generate workouts...");
    try {
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: "gpt-4o",
                messages: [
                    { role: "system", content: "You are an expert running coach. You output ONLY valid JSON. No markdown formatting." },
        ...
        processAIResponse(content, weekIndex, availability);
...
async function callGeminiAPI(prompt, weekIndex, availability) {
    showToast("✨ Asking Gemini to generate workouts...");
    try {
